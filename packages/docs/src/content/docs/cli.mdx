...
# List project memories only
codesm memory list --project

# Add a memory
codesm memory add "Use pytest for testing" --type preference

# Delete a memory
codesm memory forget <memory-id>
...
# Search the index
codesm index search "authentication handler"

# Clear the index
codesm index clear
```

See [Indexing](/indexing) for details.
